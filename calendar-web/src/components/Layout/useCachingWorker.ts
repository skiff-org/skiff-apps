...
import { ONE_HOUR_IN_MILLISECONDS } from '../../constants/time.constants';
import {
  RELOAD_BROWSER,
...
// Disable SW On Dev
const isEnabled = typeof window === 'object' && getEnvironment(new URL(window.location.origin)) !== 'local';

const getCacheServiceWorker = () => new Workbox('/sw.js', { scope: '/' });
...
export default function useCachingWorker() {
  ...
  useEffect(() => {
    if (!isEnabled || !serviceWorkerSupported) return;
  ...
  useEffect(() => {
    if (!isEnabled || !serviceWorkerSupported) return; // Disable on local and when not supported
