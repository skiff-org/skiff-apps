import { DropdownItem, Icon } from 'nightwatch-ui';
  ...
  return (
    <Drawer hideDrawer={hideDrawer} show={show} title='Move to'>
      <DrawerOptions>
        {systemLabels
          .filter((systemLabel) => {
            if (currentLabel === SystemLabels.Imports) {
              return systemLabel.value !== SystemLabels.Archive && systemLabel.value !== currentLabel;
            }
            return systemLabel.value !== currentLabel;
          })
          .map((systemLabel) => (
            <DrawerOption
