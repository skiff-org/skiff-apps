...
import { ProductApp, SubscriptionPlan, WorkspaceEventType } from 'skiff-graphql';
...
const AppSwitcher: React.FC<AppSwitcherProps> = ({ user, numUnread }) => {
        ...
        if (allUUIDs.length > 0) {
          const userAccounts = await Promise.all(
            allUUIDs.map(async (uuidElem) => {
              ...
              const numUnreadForUser = await getNumUnreadForUser(routerURI, uuidElem);
  ...
  if (activeSubscription === SubscriptionPlan.Free && !isReactNativeDesktopApp()) {
