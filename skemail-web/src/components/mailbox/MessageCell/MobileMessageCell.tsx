...
import { useTheme, useUserPreference, WalletAliasWithName } from 'skiff-front-utils';
...
import { LABEL_TO_SYSTEM_LABEL, UserLabelPlain, UserLabelAlias, UserLabelQuickAlias } from '../../../utils/label';
...
export const MobileMessageCell = ({
  ...
  const archiveOrTrashIcon = label === SystemLabels.Imports ? Icon.Trash : Icon.Archive;
  ...
  const performSwipeGesture = (swipeGestureSetting: SwipeSetting | undefined) => {
    ...
    switch (swipeGestureSetting) {
      ...
      case SwipeSetting.Delete:
        ...
        void trashThreads([threadID], false, undefined, label);
        break;
    }
  };
  ...
  const onSwipeCompleteCb = async (progress: number) => {
    if (isInboxLabel) {
      ...
      if (progress > 0) {
        ...
        } else if (isDraft || isImported) {
          // if draft or imported email, send to trash
          void trashThreads([threadID], isDraft, undefined, label);
        } else {
          void archiveThreads([threadID]);
        }
      }
