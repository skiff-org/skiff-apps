        ...
        actions={[
          {
            onClick: async () =>
              setStepPasswordChange(
                // Dynamically query `canDirectlyUpdateSrp` since it might have changed since last render (e.g. enable 2FA)
                (await canDirectlyUpdateSrp(client))
                  ? STEPS_PASSWORD_CHANGE.CHANGE_MODAL
                  : STEPS_PASSWORD_CHANGE.AUTHENTICATE_MODAL
              ),
            label: 'Update',
            type: 'button'
          }
