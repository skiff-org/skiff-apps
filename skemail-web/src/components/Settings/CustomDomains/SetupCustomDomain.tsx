...
import { PaywallErrorCode, StorageTypes, TrialOfferWithOneClickDomainFeatureFlag } from 'skiff-utils';
...
const SetupCustomDomain: React.FC<SetupCustomDomainProps> = ({ existingCustomDomains, refetchCustomDomains }) => {
  ...
  const { querySearchParams, closeSettings } = useSettings();
  ...
  // immediately open 1CCD search modal when the associated setting value
  // is navigated to explicitly
  useEffect(() => {
    if (querySearchParams.setting === SettingValue.BuyDomain) {
      dispatch(
  ...
  }, [dispatch, querySearchParams.setting]);
  ...
  const onClickBuy = () => {
    ...
    // necessary to avoid drawer/modal collision until
    // we design a mobile-optimized 1CCD search flow
    if (isMobile) closeSettings();
    dispatch(
      skemailModalReducer.actions.setOpenModal({
        type: ModalType.SearchCustomDomain
      })
  ...
  const oneClickCustomDomainBannerCopy = isFreeTrialOfferAvailable
    ? {
        ...
        body: isMobile ? '' : 'Get two months of Skiff Pro free ($20 value) when you register your perfect domain'
      }
    : {
        title: 'Get a new, personalized domain',
        body: isMobile ? '' : 'Register the perfect domain name for your email in less than 5 minutes'
      };

  // Only admins can buy one-click custom domains for a workspace
  const showBuyCustomDomains = currentUserIsOrgAdmin && !isMobileApp();
