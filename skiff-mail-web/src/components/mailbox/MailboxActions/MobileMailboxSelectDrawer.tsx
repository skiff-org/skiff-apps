import { DropdownItem, Icon, Icons, Size, ThemeMode, Typography, TypographySize } from 'nightwatch-ui';
...
const LabelItem: FC<{ label: Label }> = ({ label }) => {
  ...
  const displayLabelName = getLabelDisplayName(label.name, walletAliasesWithName);
  ...
  return (
    <Drawer hideDrawer={hideDrawer} show={show}>
      ...
      <DrawerOptions>
        ...
        {showAliasInboxes && (
          <>
            {orderedAliasLabels.length > 0 && renderLabelHeader('Addresses')}
