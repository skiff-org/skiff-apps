import { CircularProgress, Skeleton, Typography } from 'nightwatch-ui';
...
const ManageCustomDomains: React.FC<ManageCustomDomainsProps> = ({
  ...
  const sortedCustomDomains = !!customDomains
    ? [...customDomains].sort((a, b) => (a?.domain ?? '').localeCompare(b?.domain ?? ''))
    : undefined;
  return (
    <>
      <TitleActionSection
      ...
      {!loading && (!!sortedCustomDomains?.length || isPolling) && (
        <DomainsList>
          {!!sortedCustomDomains?.length &&
            sortedCustomDomains.map((customDomain) => {
