...
import {
  ...
  useRenameWebauthnDeviceMutation
...
function SetupMFA() {
  ...
  const [renameCredential] = useRenameWebauthnDeviceMutation();
  ...
  const [savedLoginSrpRequest, setSavedLoginSrpRequest] = useState<LoginSrpRequest>();
  const [editingWebauthnCredentialNameID, setEditingWebauthnCredentialNameID] = useState<
    { credentialID: string; credentialName?: string } | undefined
  >(undefined);
  ...
  const onSubmitAuthentication = async (loginSrpRequest: LoginSrpRequest) => {
    setSavedLoginSrpRequest(loginSrpRequest);
    ...
    if (!isMFAEnabled || isAddingTotpToWebauthn || isAddingWebauthnToTotp) {
      ...
      const userAlias = emailAliases[0];
      if (!userAlias) {
        return false;
      }
      const { generatedSecret, url, error: mfaConfigurationError } = await configureMFA(userAlias);
  ...
  const enableMfa = async () => {
    if (!savedLoginSrpRequest) {
      console.error('No saved login SRP request');
      return { status: RequestStatus.Failed, backupCodes: [] };
    }
    const result = (await enrollMfa(client, userData, dataMFA, savedLoginSrpRequest)) as {
  ...
  const runRenameCredential = async () => {
    if (!editingWebauthnCredentialNameID) {
      return;
    }
    await renameCredential({
      variables: {
        request: {
          credentialID: editingWebauthnCredentialNameID.credentialID,
          newName: newCredentialName
        }
      }
    });
    setEditingWebauthnCredentialNameID(undefined);
    void checkMFA();
  };
  ...
  const inputField = (
    <InputField
      ...
      error=''
      onChange={(evt) => {
        setNewCredentialName(evt.target.value);
      }}
      onKeyPress={(e: React.KeyboardEvent) => {
        if (e.key === 'Enter' && newCredentialName) {
          void runRenameCredential();
        }
      }}
  ...
  return (
    <>
      <Dialog
        ...
        open={!!editingWebauthnCredentialNameID}
        title='Rename device'
        type={DialogType.INPUT}
      >
        <ButtonGroupItem label='Rename' onClick={() => void runRenameCredential()} />
        <ButtonGroupItem
          label='Cancel'
          onClick={() => {
            setEditingWebauthnCredentialNameID(undefined);
              ...
              <MfaRightJustifiedContainer>
                {!isMobile && (
                  <IconButton
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      // set to current name for now
                      setNewCredentialName(webauthnData.keyName ?? '');
                      setEditingWebauthnCredentialNameID({
                        credentialID: webauthnData.credentialID,
                        credentialName: webauthnData.keyName ?? undefined
                      });
                    }}
                    tooltip='Rename'
                    type={Type.SECONDARY}
