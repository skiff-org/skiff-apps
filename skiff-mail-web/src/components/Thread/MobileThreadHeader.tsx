import { FilledVariant, Icon, IconButton, Size, Typography, TypographySize, TypographyWeight } from 'nightwatch-ui';
...
const calcScrollAnimatedOffset = (scrollTop: number, min: number, max: number) => {
  if (scrollTop < 0) {
    return min;
  }
...
export default function MobileThreadHeader({
  ...
  useEffect(() => {
    ...
    const updateBodyScrollOffset = (e: Event) => {
      const bodyScrollOffset = (e.target as HTMLElement).scrollTop;
