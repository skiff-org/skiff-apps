...
import { COMPACT_ITEM_HEIGHT, ITEM_HEIGHT, MOBILE_ITEM_HEIGHT } from '../constants/mailbox.constants';
import { MailboxEmailInfo } from '../models/email';
import { MailboxThreadInfo } from '../models/thread';

import { updateLabelsNumUnreadCache, updateNumUnreadAllLabelsCache } from './cache/cache';
...
export const getItemHeight = (isMobile: boolean, isCompact?: boolean) => {
  if (isMobile) {
...
export const markAllThreadsAsRead = async (read: boolean, label: string) => {
  await client.mutate<SetAllThreadsReadStatusMutation, SetAllThreadsReadStatusMutationVariables>({
    ...
    update: (cache, response) => {
      if (response.errors) {
        console.error(`Failed to mark all as read. ${response.errors.toString()}`);
        return;
      }
...
export const updateThreadAsReadUnread = async (threadIDs: Array<string>, read: boolean, labelsToUpdate: string[]) => {
  await client.mutate<SetReadStatusMutation, SetReadStatusMutationVariables>({
    ...
    update: (cache, response) => {
      ...
      updateNumUnreadAllLabelsCache(client.cache, labelsUnreadDiff);
...
export const getReplyOrForwardFromAddress = (
  ...
  const otherEmailAliasInTo = [...emailAliases, ...quickAliases].find((alias) => aliasesSet.has(alias.toLowerCase()));
...
/** Scrolls the given thread into view */
export const scrollThreadIntoView = (containerRef: RefObject<HTMLDivElement>, threadID?: string) => {
  if (!threadID) return;

  const threadElement = document.getElementById(threadID);
  if (!threadElement || !containerRef.current) return;

  const { top: threadTop, bottom: threadBottom } = threadElement.getBoundingClientRect();
  const { top: containerTop, bottom: containerBottom } = containerRef.current.getBoundingClientRect();
  const withinViewport = threadTop >= containerTop && threadBottom <= containerBottom;
  // Do nothing if the thread is in view
  if (withinViewport) return;

  // Otherwise, scroll into to view
  threadElement.scrollIntoView({ block: 'nearest' });
};
