...
import { IndexedSkemail, SearchClient } from 'skiff-front-search';
...
export async function toFromSearch(query: string) {
  return (await getSearchWorker()?.search(query, SearchClient.SKEMAIL, { fields: toFromFields })) || [];
}
...
export function getFilterTypesToHide(filters: SearchFilter[]) {
  ...
  filters.forEach((filter) => {
    const conflictingFiltersOfFilter =
      (excludedFiltersByFilterType[filter.filter.filterType] as SearchFilterType[]) ?? [];
...
export const hideFilters = (
  ...
  const SEARCH_PATH = `${MailAppRoutes.MAIL}${MailAppRoutes.SEARCH}`;
