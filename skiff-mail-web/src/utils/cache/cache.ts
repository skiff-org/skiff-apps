...
import { LabelUnreadCount, SystemLabels, UpdatedThreadLabels, UserLabel, UserLabelVariant } from 'skiff-graphql';
...
export function updateLabelsNumUnreadCache(
  ...
  diffByLabelMap.forEach((unreadDiff, label) => {
    cache.updateQuery(
      ...
      (existingNumUnread: GetNumUnreadQuery | null) => {
...
/**
 * Updates the unread count of the given labels with their paired unread diff, in the given cache.
 * Note: This function is specifically for the numUnreadAllLabels query.
 * @param cache cache to read and update
 * @param diffByLabelMap a map of label and unread diff (for this label's unread counter) pairs
 */
export function updateNumUnreadAllLabelsCache(
  cache: ApolloCache<NormalizedCacheObject>,
  diffByLabelMap: Map<string, number>
): void {
  cache.modify({
    ...
    fields: {
      unreadAllLabels(existingData: LabelUnreadCount[] = []) {
        return existingData.map((item) => {
          const unreadDiff = diffByLabelMap.get(item.label);
          if (unreadDiff !== undefined) {
            return {
              ...item,
              count: item.count + unreadDiff
            };
          } else {
            return item;
          }
...
export function removeThreadsFromCache(params: {
  ...
  // when permanently deleting from trash, remove threads from the search index
  removeThreadsFromMailboxQuery(cache, label, fragments, label === SystemLabels.Trash);
}
    ...
    if (cacheID) {
      cache.updateFragment<ThreadWithoutContentFragment>(
        ...
        (existing) => {
          ...
          const isImported = existing.attributes.userLabels.some(
            (userLabel) => userLabel.variant === UserLabelVariant.Import
          );
          const isQuickAlias = existing.attributes.userLabels.some(
            (userLabel) => userLabel.variant === UserLabelVariant.QuickAlias
          );
          ...
          if (labelsToAddThreadTo.includes(SystemLabels.Trash)) {
            ...
            if (isImported) labelsToRemoveThreadFrom.push(SystemLabels.Imports);
            if (isQuickAlias) labelsToRemoveThreadFrom.push(SystemLabels.QuickAliases);
          }
