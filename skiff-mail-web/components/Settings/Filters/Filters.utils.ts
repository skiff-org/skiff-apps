...
import {
  ConditionComparator,
  ConditionType,
  ENCRYPTED_CONDITION_TYPES_TO_DATAGRAM,
  FRONTEND_CONDITION_TYPES
...
export const createConditionID = (comparator: ConditionComparator, type: ConditionType) => {
  return `${type}-${comparator}`;
};

export const getAvailableConditionTypes = (hasFrontendMailFilteringFF: boolean) => {
  const allConditionTypes = Object.values(ConditionType);
  return !hasFrontendMailFilteringFF
    ? allConditionTypes.filter((type: ConditionType) => !FRONTEND_CONDITION_TYPES.includes(type))
    : allConditionTypes;
};
...
export const conditionsFromFilterGraphQL = (
  ...
  subFilter?.forEach((filter) => {
    ...
    // Get value
    const value = getConditionValueFromSerializedData(
      serializedData ?? '',
      conditionType,
      contacts,
      clientside,
    ...
    // Add the condition
    conditions.push({
      id: createConditionID(comparator, conditionType),
      type: conditionType,
      comparator,
